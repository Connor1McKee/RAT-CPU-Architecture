...
;r2 can be used for something else it is currently being used for debugging
...
;------------------------------------------------------
		...
		;initialize all registers to 0 to start game
		MOV r4, 0x00
		MOV r5, 0x00
		MOV r7, 0x00
		MOV r8, 0x00
		...
		;CMP r4, 0x74        	;check if right cursor movement key pressed
		;BREQ RIGHT
		;CMP r4, 0x6B        	;check if left cursor movement key pressed 
		;BREQ LEFT
		;CMP	r4, 0x29			;check if shoot key pressed
		;BREQ SHOOT
		;cmp to check for shoot from r30
		;if shot from r30
		;CALL choose color
		;update the register and draw in position
		AND r0, r0
...
;-
;-  Draws the character B from (r8,r7) to (r8,r9) using color in r6
;-
;-  Parameters:
;-   r8  = x-coordinate
;-   r7  = starting y-coordinate
...
;-   r6  = color used for line
;- 
;- Tweaked registers: r7,r9, r8, r10, r12
;--------------------------------------------------------------------
draw_character_B:
		MOV		r9, r7					; set ending Y coordinate
...
DRAW_CURSOR_90:	 
		 ;draw starting cursor 90 degrees
		 MOV r7, 0x30
		 MOV R8, 0x19
		 MOV r9, 0x35
...
RIGHT:
		...
		;ADD		r0, 0x01			
		;CMP 	r0, 0x00			; check to see if in left pos first
		;BREQ	ER_LEFT_DR_CENTER
		CMP 	r0, 0x00			; check to see if in center position
...
;---------------------------------------------------------------------
;- SHOOT - selects which angle to shoot at
;---------------------------------------------------------------------
SHOOT:
		IN r3, RANDOM_COLOR
		;CALL choose_color		
...
SHOOT_CENTER:
		MOV r7, 0x2F
		MOV r8, 0x19
		MOV r6, 0x00				; set color to randomly generated color
		CALL draw_dot
		;MOV r6, r17					; set color to randomly generated color
		;CALL draw_dot
	loop:
		...
		CALL check_for_hit
		CALL check_top_border		; checking for the top y position
		;CALL check_for_hit
...
SHOOT_LEFT:
	...
	left_loop:
		...
		CALL check_for_left_border	; checking for the top y position
		SUB r10, 0x01
		;SUB r11, 0x01
		CALL check_for_hit
...
check_for_hit:
  ...
  subsequent_hit:
	...
	one:	CALL check_color2			; check above color
											; check to see if at max number of bubbles to delete
		;CLC
		ADD r28, 0x01
		CMP  r10, r13
		BREQ two
	...
	two:
		ADD r11, 0x01 				;check diaganol right above
		CALL check_color2
								; check to see if at max number of bubbles to delete
		;CLC
		ADD r28, 0x01
	...
	three:	ADD r10, 0x01				;check right
		CALL check_color2
		...
		BREQ store_position
	;	CMP r25, 0x04
	;	BREQ delete_bubbles				; check to see if at max number of bubbles to delete
		
	four:
		ADD r10, 0x01				;check below right diag
		CALL check_color2
					; check to see if at max number of bubbles to delete
		;CLC
		ADD r28, 0x01
	...
	six:
		SUB r11, 0x01				;check left down diag
		CALL check_color2
						; check to see if at max number of bubbles to delete
		;CLC
		ADD r28, 0x01
	...
	seven:	
		...
		;BREQ delete_bubbles	
	eight:	
		SUB r10, 0x01				;check left up diag
		CALL check_color2
					; check to see if at max number of bubbles to delete
		;CLC
		ADD r28, 0x01
...
;---------------------------------------------------------------------		
	...
	delete_bubbles:
		...
		;first delete the bubble at its current position
		;the bubble's current position should be the same when it was called
		MOV r2, 0x0F
		OUT r2, SSEG_ID1 
		MOV r6, 0x00				; set color to black
	...
	loop_through_stack:
		...
		MOV r25, 0x00
		CMP r27, 0x20 				; check for end of bubbles to delete
		...
		CMP r0, 0x00
		BREQ check_center_for_nonblack
		CMP r0, 0xFF
...
check_center_for_nonblack:
		;MOV r11, r14				; restore original X value
